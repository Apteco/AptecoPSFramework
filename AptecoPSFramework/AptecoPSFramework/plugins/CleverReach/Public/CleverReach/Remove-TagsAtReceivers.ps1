...
function Remove-TagsAtReceivers {
    ...
    process {
        ...
        $receivers | group group_id | ForEach-Object {
            ...
            $groupReceivers = @( $receivers | where { $_.group_id -eq $groupId } )
            ...
            For ($i = 0 ; $i -lt $batches; $i += 1 ) {
                ...
                $upsertBody = @( $groupReceivers[$start..$end] | % { $rec =$_;[PSCustomObject]@{"email"=$rec.email;"tags"=[array]@("-$( $Source ).$( $Tag )")} } )
