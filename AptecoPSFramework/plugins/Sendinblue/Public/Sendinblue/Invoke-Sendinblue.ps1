...
function Invoke-Sendinblue {
    ...
    Begin {
        ...
        If ( $Script:variableCache.Keys -contains "sib_access_token" ) {
            ...
            $token = $Script:variableCache."sib_access_token"
        } else {
            # Try to read the token from file
            # If that failes, just proceed, it will try to create a new one
            If ( ( Test-Path -Path $Script:settings.token.tokenSettingsFile ) -eq $true ) {
                ...
                #} else {
                    ...
                    If ( $Script:variableCache.Keys -contains "sib_access_token" ) {
                        $Script:variableCache.add("sib_access_token", $token)
                    }
                #}
            }
