...
function Invoke-Optilyz {
    ...
    param (
        ...
        ,[Parameter(Mandatory=$false)][Int]$ApiVersion = 2                          # APIVersion to use, default is 2
    )
    DynamicParam {
    ...
    Begin {
        ...
        # Build up header
        $header = [Hashtable]@{
            "Authorization" = "Bearer $( ( Convert-SecureToPlaintext -String $Script:settings.login.token ) )"
        }
        ...
        # set the pagesize
        # TODO Not yet reworked for Optilyz
        If ( $Pagesize -gt 0 ) {
    ...
    Process {
        ...
        Do {
            ...
            try {
                ...
                # Prepare URL
                $uri = "$( $base )v$( $ApiVersion.toString() )/$( $object )/$( $Path )"
                if ($uri.EndsWith('/')) {
                    $uri = $uri.Substring(0, $uri.Length - 1)
                }
                $uriRequest = [System.UriBuilder]::new($uri)
