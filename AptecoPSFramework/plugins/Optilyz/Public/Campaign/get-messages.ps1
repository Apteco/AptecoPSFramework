    ...
    process {
        ...
        #Switch ( $InputHashtable.mode ) {
            ...
            #default {
                ...
                $campaigns = $automations | Select-Object @{name="aid"; expression={ $_._id }}, @{name="aname"; expression={ $_.name }} -ExpandProperty variations

                <#
                
                id         : 1
                name       : Variation A - Bibelverbreitung
                letter     : A
                customName : Bibelverbreitung
                visuals    : {6908a5797c2e484d16ada3d4}
                aid        : 6902150aa5100ed85977c07b
                aname      : Dankesschreiben

                id         : 2
                name       : Variation B - Bibel im Gefängnis
                letter     : B
                customName : Bibel im Gefängnis
                visuals    : {6908a64ba2382ed801e7c60f}
                aid        : 6902150aa5100ed85977c07b
                aname      : Dankesschreiben
                
                #>

                Write-Log "Loaded $( $automations.Count ) automations with $( $campaigns.Count ) variations" -severity INFO #-WriteToHostToo $false
                ...
                $campaigns | ForEach-Object {
                    ...
                    [void]$mailingsList.add(
                        [Mailing]@{
                            "mailingId" = "$( $mailing.aid )#$( $mailing.id )"
                            "mailingName" = "$( $mailing.aname ) - $( $mailing.name )"
                        }
                    )
                }
